...
class FileManager():
    ...
    #     bankName:str = self.bankNameExtractor.extractBankName(fileUrl)
    #     beginningsOfFields = self.patternSelector.selectPattern(bankName)
    #     tabula.io.convert_into(fileUrl,fileHolderOfDataCsv,output_format="csv",pages="all")
    #     for beginningOfField in beginningsOfFields:
    #         outputFile = os.path.join(outputDirectory, beginningOfField + ".csv") 
    #         with open(fileHolderOfDataCsv, "r") as f:
    ...
    def writeFiles(self):
        for bankName in self.files:
            for fileName in self.files[bankName]:
                ...
                # self.write(fileName+'.csv', outputDirectory);
                print(fileName)
